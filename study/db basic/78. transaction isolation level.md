...
2. non-repeatable read
  - definition
    - in one transaction read the same row twice, but the same row has  different value. because another transaction updated the row
  - ex. 
...
3. phantom read 
  - definition
    - in one transaction read the same table twice, but a new row is inserted or an existing row is deleted. because another transaction inserted or deleted
  - ex.
